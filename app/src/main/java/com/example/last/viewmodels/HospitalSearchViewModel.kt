...
import com.example.last.NearbyDonorsResponse
...
class HospitalSearchViewModel : ViewModel() {
    ...
    fun setContext(context: Context) {
        ...
        Log.d("HospitalSearchVM", "Context set for location client")
    }

    fun showToast(message: String) {
        toastMessage = message
        Log.d("HospitalSearchVM", message)
    }
        ...
        when (field) {
            "latitude" -> {
                ...
                if (currentLatitude != null && currentLongitude != null) {
                    ...
                    Log.d("HospitalSearchVM", "Current location updated: lat=$currentLatitude, lon=$currentLongitude")
                    if (isNearbySearchActive) {
                        findNearbyDonorsWithLocation()
                    }
                } else {
                    Log.d("HospitalSearchVM", "Latitude updated but location not complete: $currentLatitude")
                }
            }
            "longitude" -> {
                ...
                if (currentLatitude != null && currentLongitude != null) {
                    ...
                    Log.d("HospitalSearchVM", "Longitude updated but location not complete: $currentLongitude")
                }
            }
        }
    }
    ...
    private fun fetchLocation(onComplete: (Boolean) -> Unit = {}) {
        context?.let { ctx ->
            try {
                ...
                Log.d("HospitalSearchVM", "Fetching current location")
                fusedLocationClient.lastLocation
                    .addOnSuccessListener { location: Location? ->
                        if (location != null) {
                            showToast("Location obtained: ${location.latitude}, ${location.longitude}")
                            Log.d("HospitalSearchVM", "Location obtained: ${location.latitude}, ${location.longitude}")
                            ...
                            Log.d("HospitalSearchVM", "Location is null, requesting location updates")
                            requestLocationUpdates(onComplete)
                        }
                    }
                    .addOnFailureListener { e ->
                        showToast("Failed to get location: ${e.message}")
                        Log.d("HospitalSearchVM", "Failed to get location: ${e.message}")
                        requestLocationUpdates(onComplete)
                    }
            } catch (e: Exception) {
                showToast("Error in location fetch: ${e.message}")
                Log.d("HospitalSearchVM", "Error in location fetch: ${e.message}")
                isLoading = false
                onComplete(false)
            }
        } ?: run {
            ...
            showToast(error!!)
            Log.d("HospitalSearchVM", "Context not initialized for location fetch")
            onComplete(false)
        }
    }
    ...
    private fun requestLocationUpdates(onComplete: (Boolean) -> Unit) {
        context?.let { ctx ->
            try {
                ...
                val locationCallback = object : LocationCallback() {
                    override fun onLocationResult(locationResult: LocationResult) {
                        ...
                        locationResult.lastLocation?.let { location ->
                            ...
                            Log.d("HospitalSearchVM", "Location update received: ${location.latitude}, ${location.longitude}")
                            updateField("latitude", location.latitude.toString())
                            updateField("longitude", location.longitude.toString())
                            ...
                            Log.d("HospitalSearchVM", "Location update failed: No location received")
                            isLoading = false
                            onComplete(false)
                        }
                    }
                ...
                Log.d("HospitalSearchVM", "Requested location updates")
            } catch (e: Exception) {
                showToast("Failed to request location updates: ${e.message}")
                Log.d("HospitalSearchVM", "Failed to request location updates: ${e.message}")
                isLoading = false
                onComplete(false)
            }
        } ?: run {
            error = "Context not initialized"
            showToast(error!!)
            Log.d("HospitalSearchVM", "Context not initialized for location updates")
            onComplete(false)
        }
    }
    ...
    fun fetchDonors() {
        isLoading = true
        error = null
        Log.d("HospitalSearchVM", "Fetching donors from Firebase")

        val currentHospitalId = auth.currentUser?.uid ?: return

        contactRequestsRef.orderByChild("recipientId").equalTo(currentHospitalId)
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(requestsSnapshot: DataSnapshot) {
                    ...
                    donorsRef.addValueEventListener(object : ValueEventListener {
                        override fun onDataChange(donorsSnapshot: DataSnapshot) {
                            ...
                            Log.d("HospitalSearchVM", "Fetched ${donorsSnapshot.childrenCount} donors from Firebase")
                            ...
                            for (donorSnapshot in donorsSnapshot.children) {
                                ...
                                Log.d("HospitalSearchVM", "Donor: $name, Lat: $latitude, Lon: $longitude (from strings: $latitudeStr, $longitudeStr)")

                                if (latitude == null || longitude == null || !latitude.isFinite() || !longitude.isFinite()) {
                                    showToast("Donor $name has no valid location data")
                                    Log.d("HospitalSearchVM", "Donor $name has no valid location data: lat=$latitude (from $latitudeStr), lon=$longitude (from $longitudeStr)")
                                }
                            ...
                            Log.d("HospitalSearchVM", "Loaded ${donors.size} donors into state")

                            if (isNearbySearchActive && locationInitialized) {
                                Log.d("HospitalSearchVM", "Triggering nearby donors filter after donor fetch")
                                findNearbyDonorsWithLocation()
                            }
                        }

                        override fun onCancelled(error: DatabaseError) {
                            this@HospitalSearchViewModel.error = error.message
                            showToast(error.message)
                            Log.d("HospitalSearchVM", "Donor fetch cancelled: ${error.message}")
                            isLoading = false
                        }
                    })
                }
                ...
                override fun onCancelled(error: DatabaseError) {
                    this@HospitalSearchViewModel.error = error.message
                    showToast(error.message)
                    Log.d("HospitalSearchVM", "Contact requests fetch cancelled: ${error.message}")
                    isLoading = false
                }
            })
    }
    ...
    fun findNearbyDonors() {
        ...
        Log.d("HospitalSearchVM", "Find nearby donors triggered, isNearbySearchActive=$isNearbySearchActive")

        if (currentLatitude == null || currentLongitude == null) {
            showToast("Fetching current location for nearby search...")
            Log.d("HospitalSearchVM", "Current location not available, fetching location")
            fetchLocation { success ->
                if (success) {
                    Log.d("HospitalSearchVM", "Location fetched successfully, proceeding to API call")
                    findNearbyDonorsWithLocation()
                } else {
                    error = "Unable to get current location"
                    showToast(error!!)
                    Log.d("HospitalSearchVM", "Failed to fetch location, resetting nearby search")
            ...
            Log.d("HospitalSearchVM", "Current location available, proceeding to API call")
            findNearbyDonorsWithLocation()
        }
    }
    ...
    private fun findNearbyDonorsWithLocation() {
        if (currentLatitude == null || currentLongitude == null) {
            ...
            showToast(error!!)
            Log.d("HospitalSearchVM", "findNearbyDonorsWithLocation failed: Current location not available")
            isNearbySearchActive = false
            isLoading = false
            nearbyDonors = emptyList()
            return
        ...
        Log.d("HospitalSearchVM", "Calling API for nearby donors with location: $currentLatitude, $currentLongitude")
        ...
        viewModelScope.launch {
            try {
                ...
                Log.d("HospitalSearchVM", "API response: ${response.nearbyDonors} donors in cluster ${response.cluster}, modelUsed=${response.modelUsed}, modelStatus=${response.modelStatus}")
                ...
                val donorsWithCluster = donors.map { donor ->
                    if (donor.latitude != null && donor.longitude != null && donor.latitude.isFinite() && donor.longitude.isFinite()) {
                        try {
                            ...
                            Log.d("HospitalSearchVM", "Donor ${donor.name} assigned cluster: ${donorResponse.cluster}")
                            donor.copy(clusterGroup = donorResponse.cluster)
                        } catch (e: Exception) {
                            Log.d("HospitalSearchVM", "Error getting cluster for donor ${donor.name}: ${e.message}")
                            donor
                        }
                    } else {
                        donor
                ...
                val filteredNearbyDonors = donorsWithCluster.filter { donor ->
                    if (donor.clusterGroup != null && donor.clusterGroup == response.cluster &&
                        ...
                        Log.d("HospitalSearchVM", "Donor ${donor.name} distance: $distance km")
                ...
                Log.d("HospitalSearchVM", "Populated nearbyDonors with ${filteredNearbyDonors.size} donors in cluster ${response.cluster}")

                if (filteredNearbyDonors.isEmpty()) {
                    showToast("No donors found in your cluster within ${maxDistanceInKm}km")
                    Log.d("HospitalSearchVM", "No nearby donors found in cluster ${response.cluster} within ${maxDistanceInKm}km")
                } else {
                    filteredNearbyDonors.forEach { donor ->
                        ...
                        Log.d("HospitalSearchVM", "Nearby donor: ${donor.name}, cluster: ${donor.clusterGroup}, distance: $distance km")
                    }
                }
            } catch (e: Exception) {
                error = "API call failed: ${e.message}"
                showToast(error!!)
                Log.d("HospitalSearchVM", "API call failed: ${e.message}")
                isNearbySearchActive = false
                isLoading = false
                nearbyDonors = emptyList()
            }
    ...
    private fun calculateHaversineDistance(
        ...
        val distance = r * c
        Log.d("HospitalSearchVM", "Haversine distance calculated: $distance km between ($lat1, $lon1) and ($lat2, $lon2)")
        return distance
    }
    ...
    fun resetNearbySearch() {
        ...
        Log.d("HospitalSearchVM", "Nearby search reset, showing all donors")
    }

    fun initializeStorage(appId: String, apiKey: String, projectId: String, bucketUrl: String) {
        storage = FirebaseStorage.getInstance()
        storageReference = storage.reference
        Log.d("HospitalSearchVM", "Firebase Storage initialized")
    }
}